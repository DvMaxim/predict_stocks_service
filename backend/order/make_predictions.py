...
def process_order(order_items, source_mail, destination_mail, user_name, user_surname, order_date, order_time):
    ...
    for item in order_items:
        ...
        if product[0].product_category.category_id == "it_companies":
            ...
            if product[0].product_Name == "Nestlé":
                predicted_price = get_future_price(ticker=product[0].product_Ticker, lookup_step=quantity)
        ...
        if predicted_price:
            message += f"Продукт: {product[0].product_Name}." \
                       f"\nКатегория продукта: {product[0].product_category.category_id}.\n" \
                       f"\nКоличество дней для рассчета прогноза: {quantity}.\n" \
                       f"Результат: стоимость акции {product[0].product_Name} через {quantity} дн. составит {predicted_price:.2f}$.\n\n"
    subject = f'Прогноз пользователя {user_name} {user_surname}, заказанный {order_date}, {order_time}.'
