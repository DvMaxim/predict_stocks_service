...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, first_Name, last_Name, email, password, phone_Number):
        type = "ADMIN"
        user = self.create_user(first_Name, last_Name, email, password, phone_Number, type)
    ...
    def create_user(self, first_Name, last_Name, email, password, phone_Number, type):
        if not email:
            raise ValueError(_('You must provide an email address'))

        email = self.normalize_email(email)
        user = self.model(email=email, first_Name=first_Name, last_Name=last_Name, phone_Number=phone_Number, type=type)
